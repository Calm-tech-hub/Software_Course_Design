...
public class HoneyBee extends Bee{
	...
	public void search(){
	    ...
	    int n_findbee_me=0;
	    int[] id_findbee_me = new int[20];
	    double[] posangle_findbee_me = new double[20];
	    double[] flyangle_findbee_me = new double[20];
	    ...
	    while(m.find()) {
	    		...
	    		{
	    			n_findbee_me++;
	    			id_findbee_me[n_findbee_me] = Integer.parseInt(cc[0]);
		    		posangle_findbee_me[n_findbee_me] = correctvalue(Double.parseDouble(cc[1]));
		    		flyangle_findbee_me[n_findbee_me] = correctvalue(Double.parseDouble(cc[2]));
//		    		if (id_findbee_me[n_findbee_me]==id)
//					{
...
//					}
	    ...
	    if (isDanger==false && n_findflower>0)
		{
			k=1;
	    ...
	    {   ////防止被追赶时遇到花自动采蜜
			for(int f=0; f<n_findflower;f++)
			{
				if((angle_findflower[f]-runangle>-8)||(angle_findflower[f]-runangle<8))
				{
					runangle=correctvalue(runangle+15);
				}
			}
	    	angle = runangle;
	    }
	    ...
	    {
	    	/////如果和前面的蜜蜂飞行的角度很相似，转90度，
	    	for (i=1;i<=n_findbee_me;i++)
	    		if(flyangle_findbee_me[i]!=id)
	    		{
	    			if( (flyangle_findbee_me[i]-angle<15) && (flyangle_findbee_me[i]-angle>-15) )
	    			{
	    				Random ra1 = new Random();
	    				angle += ra1.nextInt(90);
	    				angle = correctvalue(angle);
	    			}
	    		}
