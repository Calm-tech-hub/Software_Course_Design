...
public class BeeFarming extends JFrame{
	...
	*/
	...
	{
		// 在headless模式下(GameEngine运行时),转发到GameEngine
		try {
			Class<?> geClass = Class.forName("GameEngine");
			java.lang.reflect.Field currentEngineField = geClass.getDeclaredField("currentEngine");
			currentEngineField.setAccessible(true);
			Object currentEngine = currentEngineField.get(null);
			if (currentEngine != null) {
				java.lang.reflect.Method method = geClass.getMethod("update", FlyingStatus.class);
				method.invoke(null, fs);
				return;
			}
		} catch (Exception e) {
			// 继续使用原逻辑
	...
	*/
	public static String search(int id)
	{
		...
		try {
			...
			if (currentEngine != null) {
				// GameEngine正在运行,转发请求
				java.lang.reflect.Method method = geClass.getMethod("search", int.class);
				return (String) method.invoke(null, id);
			}
		} catch (Exception e) {
			// GameEngine不可用,继续使用原逻辑
		}
	...
	*/
	public static int pickFlowerHoney(int id){
		...
		try {
			...
			Object currentEngine = currentEngineField.get(null);
			if (currentEngine != null) {
				java.lang.reflect.Method method = geClass.getMethod("pickFlowerHoney", int.class);
				return (int) method.invoke(null, id);
			}
		} catch (Exception e) {
			// 继续使用原逻辑
		}
	...
	*/
	...
	public static void killBee(int id){
		...
		try {
			...
			Object currentEngine = currentEngineField.get(null);
			if (currentEngine != null) {
				java.lang.reflect.Method method = geClass.getMethod("killBee", int.class);
